...
- Update [`docs/overview/roadmap.md`](../../00-overview/roadmap.md) planning posture if required when Phase 11 moves from “planned only” to “has approved ticket decomposition.”
- Update [`README.md`](../../../README.md) if user-visible config (`tmdb` block) or operator steps change.
- Update [`docs/overview/start-here.md`](../../00-overview/start-here.md) only if the “current phase” pointer or next-action for contributors changes materially.
- Manual checklist against [`docs/product/plans/phase-11-tmdb-metadata-enrichment.md`](../../01-product/phase-11-tmdb-metadata-enrichment.md) **Exit Condition** and **Explicit Deferrals**.
...
- `docs/overview/roadmap.md`: Phase 10 marked implemented on `main`; Phase 11 marked implemented via stacked delivery with `closeout-stack` merge note; planning posture updated; Phase 10 deferral text adjusted so TMDB is attributed to Phase 11.
- `docs/overview/start-here.md`: current state through Phase 10 on `main` and Phase 11 in the stack; dashboard + TMDB surfaces; deferred list no longer claims “web UI” is missing.
- `README.md`: Phase 11 / closeout pointer, TMDB and `tmdbRefreshIntervalMinutes` in config shape, dashboard subsection.

**Exit condition checklist** (vs `docs/product/plans/phase-11-tmdb-metadata-enrichment.md`):
