...
## Post-P29 Follow-Up: Native Provider Credential Mode

Recorded 2026-08-27, after the P29 stack was complete and awaiting closeout. **Not P29 scope** — captured here so the finding is not lost, to be picked up as a standalone PR or folded into a later phase.

P29 ships exactly one credential path: upload an `.ovpn` profile, with `provider` pinned to `custom_openvpn` and `src/vpn-api.ts` emitting `VPN_SERVICE_PROVIDER: custom`. That is correct and sufficient for BYO OpenVPN, but it is the harder of the two paths gluetun actually offers.

Gluetun natively supports roughly forty providers (Proton VPN, NordVPN, Mullvad, AirVPN, Surfshark, Windscribe, and others). For those it resolves servers from a built-in list and **no profile file exists** — configuration is a provider name plus a credential. An owner on Proton or Nord currently has to go find and upload an artifact gluetun never consumes.

A follow-up would add a second credential mode alongside the existing one:

- Owner picks a provider from gluetun's supported set; no file upload
- Maps to `VPN_SERVICE_PROVIDER=<name>` plus `OPENVPN_USER` / `OPENVPN_PASSWORD`
- Optional server filtering via `SERVER_COUNTRIES` / `SERVER_CITIES`
- `downloaderNetwork` gains a `credentialMode` discriminator; the existing upload path becomes the second branch
- Storage contract is unchanged — secrets stay in the daemon-owned mounted file, out of `pirate-claw.config.json` and out of generated Compose YAML

Two related findings worth carrying forward:

- **Port forwarding should be an explicit toggle.** Gluetun automates it where the provider supports it (`VPN_PORT_FORWARDING=on`; Proton additionally wants `+pmp` on the OpenVPN username, or NAT-PMP enabled when generating a WireGuard key). Torrent throughput is materially worse without it, so leaving it implicit ships a quietly degraded downloader.
- **Free VPN tiers do not work for this product's purpose.** Proton VPN's free plan is what owners will reach for first, and it forbids P2P: the tunnel establishes, `vpn_bridge_active` verification passes, and torrents silently stall. That failure mode is bad enough that the product should name it rather than let the owner debug it. This is provider policy across the industry, not a gluetun gap — there is no free path to P2P-permitting egress.

The **WireGuard deferral should also be revisited** at that time. It was deferred on the grounds that it "requires a different profile upload UX" — true for custom profiles, but under a native provider mode WireGuard is a single `WIREGUARD_PRIVATE_KEY` string and is _simpler_ than the OpenVPN path, not harder. The stated rationale would no longer hold.
