...
- For phase work, read `docs/overview/start-here.md` and `docs/engineering/delivery-orchestrator.md` first, then surface the orchestrator path before coding.
...
- Prefer `bun run deliver --plan ...` over ad hoc implementation. The delivery orchestrator reads `orchestrator.config.json` at repo root; see `docs/engineering/delivery-orchestrator.md`. For orchestrated ticket work, the handoff under `.agents/delivery/<plan-key>/handoffs/` is required input alongside plan and ticket docs.
- `codex-preflight` is a standalone pre-push code-quality gate — it is **not** the orchestrator's `subagentReview` step and must not be confused with it. Running `codex-preflight` requires invoking the `codex:codex-rescue` skill. Recording `clean` without running the skill is a policy violation visible in the commit trail (no `[codexPreflight]` commit). `codex-preflight clean` requires a note summarizing what Codex reviewed and concluded — no note is a policy violation visible in the PR body.
- The orchestrator's `subagentReview` step uses a **Claude subagent** (same execution agent type as the parent session). Never use `codex:codex-rescue` for `subagentReview` — Codex is only for the standalone `codex-preflight` gate.
...
- Check `docs/overview/start-here.md` when delivered scope, commands, status, or deferrals changed.
- Check `docs/overview/roadmap.md` when delivered scope, status, active phase, notes, or deferrals changed.
