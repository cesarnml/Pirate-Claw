...
- The orchestrator's `subagentReview` step (pre-PR code review) uses a **Claude subagent** — same execution agent type as the parent session. Never delegate `subagentReview` to `codex:codex-rescue`.
