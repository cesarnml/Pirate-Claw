...
- The orchestrator's `subagentReview` step (pre-PR code review) launches a subagent of the same type as the execution agent by default. If `orchestrator.config.json` sets `reviewSubagentOverride`, that value is canonical and dictates what is used instead.
