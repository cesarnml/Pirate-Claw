...
- The orchestrator's `subagentReview` step (pre-PR code review) launches a subagent of the same type as the execution agent by default. If `orchestrator.config.json` sets `reviewSubagentOverride`, that value is canonical and dictates what is used instead. The subagent prompt must be **adversarial**: assume the implementation has holes and find them — do not verify the author's stated intent. Explicitly instruct the subagent: _"Do not rationalize away anything you notice. Flag it and let the human decide."_ A checklist of 'did the ticket spec land?' is not a review.
