...
- The orchestrator's `subagentReview` step (pre-PR code review) uses a **Claude subagent** by default — same execution agent type as the parent session. If `orchestrator.config.json` sets `reviewSubagentOverride` (e.g. `"codex:codex-rescue"`), that value is canonical and must be used instead.
