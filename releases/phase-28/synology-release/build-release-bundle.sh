...
SOURCE_DIR="$ROOT_DIR/releases/phase-28/synology-release"
...
if [[ ! -f "$DAEMON_IMAGE_TAR" || ! -f "$WEB_IMAGE_TAR" || ! -f "$TRANSMISSION_IMAGE_TAR" ]]; then
  "$SOURCE_DIR/build-image-tarballs.sh"
fi
...
cp "$SOURCE_DIR/compose.synology.yml" "$BUNDLE_DIR/compose.synology.yml"
cp "$SOURCE_DIR/compose.synology.cm.yml" "$BUNDLE_DIR/compose.synology.cm.yml"
